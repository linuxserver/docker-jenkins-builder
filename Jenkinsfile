pipeline {
  ...
  stages {
    ...
    stage('Pull Request Comment Test') {
      when {
        not {environment name: 'CHANGE_ID', value: ''}
    ...
    script {
        def get_json() {
            //def url = "$CI_JSON_URL"
            def CI_URL="https://fakedomain.fake"
            def url="https://gilbnlsio2.s3.us-east-1.amazonaws.com/linuxserver/lidarr/latest/report.json"
            def SHELLCHECK_URL="https://fakedomain.fake"
            def response = sh(script: "curl -s \"$url\"", returnStdout: true).trim()
            if (response) {
                def json = sh(script: "echo '${response}' | jq .", returnStdout: true).trim()
                if (json) {
                    return json
                } else {
                ...
                error "Failed to retrieve JSON data from $url"
            }
        }
        def build_table(data) {
            def keys = sh(script: "echo '${data}' | jq -r 'to_entries | map(.key) | .[]'", returnStdout: true).trim()
            if (keys) {
                def header = "| Tag | Passed |\n| --- | --- |\n"
                def rows = ""
                keys.tokenize().each { build ->
                    def status = sh(script: "echo '${data}' | jq -r '.[\"${build}\"].test_success'", returnStdout: true).trim()
                    status = status == 'true' ? ':white_check_mark:' : ':x:'
                    def row = "| ${build} | ${status} |\n"
                    rows += row
                }
                def table = header + rows
                return table.replaceAll(/"/, '\\\\"')
            } else {
                error "JSON report data does not contain any keys or the report does not exist."
            }
        }
        def data = get_json()
        def table = build_table(data)
        def payload = "{\"body\":\"I am a bot, here are the test results for this PR: \\n${CI_URL}\\n${SHELLCHECK_URL}\\n${table}\"}"
        def apiEndpoint = "https://api.github.com/repos/${LS_USER}/${LS_REPO}/issues/${PULL_REQUEST}/comments"
