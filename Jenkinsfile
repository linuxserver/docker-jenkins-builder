    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              set -e
              TEMPDIR=$(mktemp -d)
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=development -v ${TEMPDIR}:/ansible/jenkins jenkinslocal:${COMMIT_SHA}-${BUILD_NUMBER} 
      ...
      steps {
        sh '''#! /bin/bash
              ...
              if [ "$(md5sum ${TEMPDIR}/package_versions.txt | cut -c1-8 )" != "${PACKAGE_TAG}" ]; then
                ...
                cd ${TEMPDIR}/${LS_REPO}/
                git --git-dir ${TEMPDIR}/${LS_REPO}/.git checkout development
