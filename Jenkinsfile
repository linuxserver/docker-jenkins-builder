pipeline {
  ...
  environment {
    ...
    DOCKERHUB_IMAGE='lsiodev/jenkins-builder'
  ...
  stages {
    ...
    stage("Set ENV Variables base"){
      steps{
        script{
          env.EXIT_STATUS = ''
          env.LS_RELEASE = sh(
            script: '''docker run --rm alexeiled/skopeo sh -c 'skopeo inspect docker://docker.io/'${DOCKERHUB_IMAGE}':looptest 2>/dev/null' | jq -r '.Labels.build_version' | awk '{print $3}' | grep '\\-ls' || : ''',
    ...
    // If this is a looptest build use live docker endpoints
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=looptest -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
      ...
      steps {
        sh '''#! /bin/bash
              ...
              if [ "${NEW_PACKAGE_TAG}" != "${PACKAGE_TAG}" ]; then
                ...
                git --git-dir ${TEMPDIR}/${LS_REPO}/.git checkout -f looptest
      ...
      steps {
        withCredentials([
          ...
          sh '''#! /bin/bash
             echo $DOCKERPASS | docker login -u $DOCKERUSER --password-stdin
             '''
          sh "docker tag ${IMAGE}:${META_TAG} ${IMAGE}:looptest"
          sh "docker push ${IMAGE}:looptest"
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:amd64-${META_TAG} ${IMAGE}:amd64-looptest"
          sh "docker tag ${IMAGE}:arm32v7-${META_TAG} ${IMAGE}:arm32v7-looptest"
          sh "docker tag ${IMAGE}:arm64v8-${META_TAG} ${IMAGE}:arm64v8-looptest"
          sh "docker push ${IMAGE}:amd64-${META_TAG}"
          sh "docker push ${IMAGE}:arm32v7-${META_TAG}"
          sh "docker push ${IMAGE}:arm64v8-${META_TAG}"
          sh "docker push ${IMAGE}:amd64-looptest"
          sh "docker push ${IMAGE}:arm32v7-looptest"
          sh "docker push ${IMAGE}:arm64v8-looptest"
          sh "docker manifest push --purge ${IMAGE}:looptest || :"
          sh "docker manifest create ${IMAGE}:looptest ${IMAGE}:amd64-looptest ${IMAGE}:arm32v7-looptest ${IMAGE}:arm64v8-looptest"
          sh "docker manifest annotate ${IMAGE}:looptest ${IMAGE}:arm32v7-looptest --os linux --arch arm"
          sh "docker manifest annotate ${IMAGE}:looptest ${IMAGE}:arm64v8-looptest --os linux --arch arm64 --variant v8"
          ...
          sh '''docker rmi \
                ...
                ${IMAGE}:amd64-looptest \
                ${IMAGE}:arm32v7-${META_TAG} \
                ${IMAGE}:arm32v7-looptest \
                ${IMAGE}:arm64v8-${META_TAG} \
                ${IMAGE}:arm64v8-looptest \
                lsiodev/buildcache:arm32v7-${COMMIT_SHA}-${BUILD_NUMBER} \
                lsiodev/buildcache:arm64v8-${COMMIT_SHA}-${BUILD_NUMBER} || :'''
        }
      }
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}' to looptest",\
             "type": "commit",\
